...
export const TodoItem = ({ todo, toggleTodo }: Props) => {
  ...
  const onToggleTodoOptimistic = async () => {
    try {
      startTransition(()=> toggleTodoOptimistic(!todoOptimistic.complete)) // Pongo esto para arreglar error cuando implemento useOptimistic
    ...
    <>
      <div className={todoOptimistic.complete ? styles.todoDone : styles.todoPending}>
        <div className='flex flex-col sm:flex-row justify-start items-center gap-4'>
          <div
            // onClick={() => toggleTodo(todoOptimistic.id, !todoOptimistic.complete)} //! Esto es sin el Optimistic
            onClick={onToggleTodoOptimistic} //Esto se hizo con el optimistic
